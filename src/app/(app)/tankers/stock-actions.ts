...
export async function createBulkPurchase(formData: FormData) {
  const returnPath = "/tankers/stock";
  ...
  try {
    ...
    if (!sourceVendor || !purchaseDate || !volumeLiters || volumeLiters <= 0 || Number.isNaN(amount)) {
      redirectWithError(returnPath, "Source/vendor, date, volume, and amount are required");
    }
...
export async function createTankerFill(formData: FormData) {
  ...
  try {
    ...
    if (!purchaseId || !toTankerId || !volumeLiters || volumeLiters <= 0) {
      redirectWithError(returnPath, "Purchase, tanker, and volume are required");
    }
    ...
    if (!purchase) redirectWithError(returnPath, "Bulk purchase not found");
    if (!tanker || !tanker.isActive) redirectWithError(returnPath, "Distribution tanker not found");
    if (tanker.tankerClass !== "DISTRIBUTION") {
      redirectWithError(returnPath, "Water can only be filled into distribution tankers");
    }
