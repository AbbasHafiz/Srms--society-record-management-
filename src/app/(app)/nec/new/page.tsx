...
import { createNecApplication } from "../actions";
...
export default async function NewNecPage({
  searchParams,
}: {
  searchParams: Promise<{ plotId?: string; q?: string }>;
}) {
  const session = await auth();
  if (!session?.user || !canCreateNecApplication(session.user.role)) {
    redirect("/nec");
  }
  ...
  const [plots, necFee] = await Promise.all([
  ...
  return (
    <div>
      <PageHeader
        title="Apply for NEC"
        description="No Encumbrance Certificate — certifies the plot is free of society encumbrances."
        actions={
          <Link href="/nec" className="text-sm text-teal-800 hover:underline">
            Back to NEC register
          </Link>
        }
      />
              ...
              >
                <div className="flex flex-col gap-4 lg:flex-row lg:justify-between">
                  <div className="space-y-2">
                    ...
                    {necFee ? (
                      <p className="text-sm text-slate-600">
                        NEC fee: <strong>{formatCurrency(necFee.amount)}</strong>
                      </p>
                    ) : null}
                  </div>

                  {owner ? (
                    <form action={createNecApplication} className="max-w-md space-y-3 lg:min-w-[20rem]">
